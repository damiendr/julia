...
begin
    ...
    L = full(Base.LinAlg.chol!(LowerTriangular, X*X'))
    U = full(Base.LinAlg.chol!(UpperTriangular, X*X'))
    XX = full(X*X')
...
for elty in (Float32, Float64, Complex{Float32}, Complex{Float64})
    ...
    @test_approx_eq full(cholfact(A)[:L]) full(invoke(Base.LinAlg.chol!, Tuple{Type{LowerTriangular}, AbstractMatrix}, LowerTriangular, copy(A)))
    @test_approx_eq full(cholfact(A)[:U]) full(invoke(Base.LinAlg.chol!, Tuple{Type{UpperTriangular}, AbstractMatrix}, UpperTriangular, copy(A)))
end
