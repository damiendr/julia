...
# instead of the properly-rounded 0.30000000000000004; check after calling fma
    if (fma_llvm(1.0000305f0, 1.0000305f0, -1.0f0) == 6.103609f-5 &&
        ...
        Base.fma(x::Float32, y::Float32, z::Float32) = fma_llvm(x,y,z)
        Base.fma(x::Float64, y::Float64, z::Float64) = fma_llvm(x,y,z)
    else
        Base.fma(x::Float32, y::Float32, z::Float32) = fma_libm(x,y,z)
        Base.fma(x::Float64, y::Float64, z::Float64) = fma_libm(x,y,z)
    end
