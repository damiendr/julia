...
function chol!{T<:BlasFloat}(::Type{UpperTriangular}, A::StridedMatrix{T})
    C, info = LAPACK.potrf!('U', A)
    return @assertposdef UpperTriangular(C) info
end
function chol!{T<:BlasFloat}(::Type{LowerTriangular}, A::StridedMatrix{T})
    C, info = LAPACK.potrf!('L', A)
    return @assertposdef LowerTriangular(C) info
end
chol!(A::StridedMatrix) = chol!(UpperTriangular, A)

function chol!{T}(::Type{UpperTriangular}, A::AbstractMatrix{T})
    n = chksquare(A)
    @inbounds begin
        for k = 1:n
            for i = 1:k - 1
            ...
            Akk = chol!(UpperTriangular, A[k,k])
            A[k,k] = Akk
            AkkInv = inv(Akk')
            for j = k + 1:n
                for i = 1:k - 1
...
function chol!{T}(::Type{LowerTriangular}, A::AbstractMatrix{T})
    n = chksquare(A)
    @inbounds begin
        for k = 1:n
            for i = 1:k - 1
            ...
            Akk = chol!(LowerTriangular, A[k,k])
            A[k,k] = Akk
            AkkInv = inv(Akk)
            for j = 1:k
                for i = k + 1:n
...
"""
chol(x::Number, args...) = chol!(nothing, x)
...
function _cholfact!{T<:BlasFloat}(A::StridedMatrix{T}, ::Type{Val{false}}, uplo::Symbol=:U)
    if uplo == :U
        return Cholesky(chol!(UpperTriangular, A).data, uplo)
    else
        return Cholesky(chol!(LowerTriangular, A).data, uplo)
    end
end
...
function cholfact!(A::StridedMatrix, uplo::Symbol, ::Type{Val{false}})
    if uplo == :U
        Cholesky(chol!(UpperTriangular, A).data, 'U')
    else
        Cholesky(chol!(UpperTriangular, A).data, 'U')
    end
end
